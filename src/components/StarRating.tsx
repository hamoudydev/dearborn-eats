    ...
    <div className="flex items-center gap-1">
      {Array.from({ length: maxRating }, (_, i) => {
        ...
        return (
          <svg
            key={i}
            className={`${sizeClasses[size]} ${isFilled ? 'text-warning' : isHalfFilled ? 'text-warning' : 'text-base-300'}`}
      ...
      {showValue && <span className="ml-1 font-medium">{rating.toFixed(1)}</span>}
    </div>
  )
}
